...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete tortoisecoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no tortoisecoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old tortoisecoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected tortoisecoin 0.7 datadir."
    ;;
  3)
    echo "Detected tortoisecoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected tortoisecoin 0.8 datadir."
    ;;
esac
