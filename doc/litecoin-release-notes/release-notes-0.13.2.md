tortoisecoin Core version 0.13.2 is now available from:

  <https://download.tortoisecoin.org/tortoisecoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/tortoisecoin-project/tortoisecoin/issues>
...
tortoisecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
tortoisecoin Core 0.13.2 also introduces new default policy limits on the length and
...
- Adding `dbcache=100` in tortoisecoin.conf
...
tortoisecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/tortoisecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The tortoisecoin Core code base has
...
- `tortoisecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `tortoisecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the tortoisecoin scripting system that in combination with
...
HD wallets are incompatible with older versions of tortoisecoin Core.
...
The two phases are distinct in the tortoisecoin-Qt GUI. During the first one,
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ...
  currently being researched by Bitcoin and tortoisecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the tortoisecoin
client to tortoisecoin Core 0.13.2 to signal for segwit support. When a
...
Since tortoisecoin Core 0.10.0, nodes have defaulted to only relaying and
