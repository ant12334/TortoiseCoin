...
*Setup instructions for a Gitian build of tortoisecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the tortoisecoin
...
- [Building tortoisecoin Core](#building-tortoisecoin-core)
...
Clone the git repositories for tortoisecoin and Gitian.
...
in the tortoisecoin repository under 'Fetch and create inputs' to install sources which require
...
To build tortoisecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tortoisecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tortoisecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/tortoisecoin-project/tortoisecoin
...
For example:
```bash
URL=https://github.com/thrasher-/tortoisecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tortoisecoin=${COMMIT} --url tortoisecoin=${URL} ../tortoisecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tortoisecoin=${COMMIT} --url tortoisecoin=${URL} ../tortoisecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tortoisecoin=${COMMIT} --url tortoisecoin=${URL} ../tortoisecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the tortoisecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tortoisecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/tortoisecoin
SIGPATH=/some/root/path/tortoisecoin-detached-sigs

./bin/gbuild --url tortoisecoin=${BTCPATH},signature=${SIGPATH} ../tortoisecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tortoisecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tortoisecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tortoisecoin-osx-build.assert
```
...
[tortoisecoin-project/gitian.sigs.ltc](https://github.com/tortoisecoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
